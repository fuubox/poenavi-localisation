...
差分: Awakenedのbase percentile、block、Memory Strandsは未対応。クラスタージュエルの
item level帯正規化やMagic JewelのAdorned向け厳密条件も未対応。
...
差分: Watcher's Eye未鑑定品のitem level固定、Agnerodのitem level帯、Magic Jewelの
corrupted exactなど、Awakenedの固有例外は未対応。
...
3. Jewel／Cluster Jewel／Gemの高頻度条件を実装
