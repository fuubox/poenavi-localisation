...
※PoE1のひな型ガイド情報は、過去に作成した攻略チャートをベースにしています。全体の流れを見たい場合はこちらをご確認ください。
...
### ⏱️ RTA タイマー
- PoE1: Act 1〜10、PoE2: Act 1〜4 + 幕間1〜3 + クリア のラップタイム計測
...
| ファイル | 説明 | Git管理 |
|----------|------|---------|
| `config.json` | ユーザー設定（ホットキー、ログパス、画面位置、タイマー状態など） | ✕（初回起動時に自動生成） |
| `data/zone_data.json` | PoE1/PoE2のエリア一覧・街エリア一覧 | ✓（マスタデータ） |
| `guide_data.json` | PoE1攻略ガイドデータ | ✓（マスタデータ） |
| `guide_data_poe2.json` | PoE2攻略ガイドデータ | ✓（マスタデータ） |
| `monster_levels.json` | エリア別モンスターレベル | ✓（マスタデータ） |
| `notes_poe1.json` / `notes_poe2.json` | ゲーム中メモ | ✕ |
| `runs/` | ラン記録 | ✕ |
| `maps/` | マップ画像 | ✓（同梱） |

`config.json` やメモ・ラン記録はユーザー環境ごとに生成されます。GitHubから取得して実行する場合も、初回起動時に必要な設定ファイルが自動作成されます。
